...
class Datatables {
     ...
     * @param mixed $key_condition
     * @param string $val
     * @param bool $escaping_content
     * @return mixed
     */
    public function having($key_condition, $val = NULL, $escaping_content = TRUE) {
        $this->having[] = array($key_condition, $val, $escaping_content);
        $this->ci->db->having($key_condition, $val, $escaping_content);
        return $this;
    }
     ...
     * @return string
     */
    public function generate($include_data = [], $output = 'json', $charset = 'UTF-8') {
        ...
        return $this->produce_output($include_data, strtolower($output), strtolower($charset));
    }
    ...
    private function get_filtering() {
        ...
        if ($sSearch != '') {
            $sSearch = $this->ci->db->escape_like_str($sSearch);
            for ($i = 0; $i < count($mColArray); $i++) {
                if ($mColArray[$i]['searchable'] == 'true')
                    if ($this->check_cType()) {
                        //$this->ci->db->or_like($this->select[$mColArray[$i]['data']], $sSearch);
                        $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' OR ";
                    } else {

                        //$this->ci->db->or_like($this->select[$this->columns[$i]], $sSearch);
        ...
        //========= CUSTOM GENERAL FILTERING
        $inFilter = $this->ci->input->post("customGeneralWhInSearch");
        //var_dump($this->select);
        if (isset($inFilter)) {
            $cSearch = explode(",", $inFilter["value"]);
            $this->ci->db->where_in($inFilter["column"], $cSearch);
        }

        //======= Individual column filtering
        //======= go through the columns adding where clause if set on a specific column.
        //======= if one filter is in eqSearch param operator changes to =

        /**/
        $eqSearchArray = $this->ci->input->post('eqSearch');
        $whInSearchArray = $this->ci->input->post('whInSearch');

        for ($i = 0; $i < count($mColArray); $i++) {

            if ($mColArray[$i]['searchable'] == 'true' && isset($mColArray[$i]['search']['value']) && $mColArray[$i]['search']['value'] != '') {
                if (isset($eqSearchArray) && in_array($this->select[$mColArray[$i]['data']], $eqSearchArray)) {
                    $cSearch = (trim($mColArray[$i]['search']['value']));
                    $this->ci->db->where($this->select[$mColArray[$i]['data']], $cSearch);
                } else {
                    if (isset($whInSearchArray) && in_array($this->select[$mColArray[$i]['data']], $whInSearchArray)) {
                        $cSearch = explode(",", (trim($mColArray[$i]['search']['value'])));
                        $this->ci->db->where_in($this->select[$mColArray[$i]['data']], $cSearch);
                    } else {
                        $cSearch = (trim($mColArray[$i]['search']['value']));
                        $this->ci->db->like($this->select[$mColArray[$i]['data']], $cSearch);
                    }
                }
            }
        }

        //======= CUSTOM SIMPLE RANGE DATE FILTER
        $simpleRangeDateSearch = $this->ci->input->post('simpleRangeDateSearch');
        if ($simpleRangeDateSearch) {
            $start_date = $simpleRangeDateSearch["start_date"];
            $end_date = $simpleRangeDateSearch["end_date"];
            $compareColumn = $simpleRangeDateSearch["compare_column"];
            $allow_null = $simpleRangeDateSearch["allow_null"];
            if (isset($this->select[$compareColumn])) {
                $start_date = $this->ci->db->escape($start_date);
                $end_date = $this->ci->db->escape($end_date);
                if ($allow_null) {
                    $this->ci->db->where("(" . $this->select[$compareColumn] . " >= " . $start_date . " and " . $this->select[$compareColumn] . " <= " . $end_date . " or " . $this->select[$compareColumn] . " is null" . ")");
                } else {
                    $this->ci->db->where("(" . $this->select[$compareColumn] . " >= " . $start_date . " and " . $this->select[$compareColumn] . " <= " . $end_date . ")");
                }
            } else {
                throw new Exception("$compareColumn column not found. ");
            }
        }

        //======== EXTERNAL SEARCHS
        $externalSearch = $this->ci->input->post('externalSearchs');
        if ($externalSearch) {
            foreach ($externalSearch as $key => $v) {
                $compareColumn = $v["compare_column"];
                $value = $v["value"];
                if (isset($this->select[$compareColumn])) {
                    if (!isset($v["active"]) || $v["active"] == "1") {
                        if ($value === "" && $v["apply_null"] === "apply_null") {
                            $this->ci->db->where($this->select[$compareColumn] . " is null");
                        } else {
                            if ($value === "" && $v["apply_null"] === "apply_not_null") {
                                $this->ci->db->where($this->select[$compareColumn] . " is not null");
                            } else {

                                $this->ci->db->where($this->select[$compareColumn], $value);
                            }
                        }
                    }
                } else {
        ...
        $externalWhSearch = $this->ci->input->post('externalWhInSearchs');
        if ($externalWhSearch) {
            foreach ($externalWhSearch as $key => $v) {
                $compareColumn = $v["compare_column"];
                $value = $v["value"];
                if (isset($this->select[$compareColumn])) {
                    if (!isset($v["active"]) || $v["active"] == "1") {
                        if ($value === "apply_null") {
                            ...
                            if ($value === "apply_not_null") {
                                $this->ci->db->where($this->select[$compareColumn] . " is not null");
                            } else {
                                $cSearch = explode(",", trim($value));
                                $this->ci->db->where_in($this->select[$compareColumn], $cSearch);
                            }
                        }
                    }
                } else {
    ...
    /**
     * Compiles the select and return sql string
     *
     * @return mixed
     */
    public function get_compiled_select($table) {
     ...
     * @return mixed
     */
    private function produce_output($include_data, $output, $charset) {
        $aaData = array();
