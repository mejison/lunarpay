...
class Ion_auth_model extends CI_Model {
    ...
    public function register($identity, $password, $email, $additional_data = [], $groups = []) {
        ...
        $user_data['first_name'] = ucwords(strtolower($user_data['first_name']));
        $user_data['last_name'] = ucwords(strtolower($user_data['last_name']));
        
        $bytes                     = openssl_random_pseudo_bytes(48, $cstrong);
        $user_data['access_token'] = bin2hex($bytes);
        ...
        //Login
        if(isset($id) && !isset($user_data['parent_id'])){//====== if parent id is set it means an internal user creation was performed we do not have to set sessions
            $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, first_name, last_name, created_on, parent_id, permissions, payment_processor')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
    ...
    public function directaccess($token, $opt = false) {
        $this->trigger_events('pre_login');
        ...
        if ($query->num_rows() === 1) {
            ...
            $this->set_session($user, $opt);

            $this->update_last_login($user->id);

            $this->clear_login_attempts($user->{$this->identity_column});
            $this->clear_forgotten_password_code($user->{$this->identity_column});
    ...
    public function add_to_group($group_ids, $user_id = FALSE) {
        ...
        foreach ($group_ids as $group_id) {
            ...
            if ($this->db->insert($this->tables['users_groups'], [$this->join['groups'] => (float) $group_id,
     ...
     * @author jrmadsen67
     */
    public function set_session($user, $opt1 = false) {
        $this->trigger_events('pre_set_session');
        
        //Do things in name of the parent if is a child //everyone likes to overwrite id so we'll use user_id
        ...
        $session_data = [
            ...
            'hide_intercom' => $opt1 == 'support' ? true : false,
            'is_child' => $user->parent_id ? true : false, //is a child account,
            'child_id' => $user->parent_id ? $user->id : null, //Do things in name of the parent if is a child //everyone likes to overwrite id so we'll use user_id
            'permissions' => strlen($user->permissions) ? explode(',', $user->permissions) : [],
            'payment_processor_short' => $user->payment_processor
        ];
            
        $this->session->set_userdata($session_data);
        
        $this->load->model('organization_model');
        $orgnx = $this->organization_model->getFirst($user_id, 'ch_id');
        
        if ($orgnx) { //login, direct access etc ... we expect there is a created organization, 1 at least            
            $this->setUserCurrentOrganization($orgnx->ch_id, 'org'); 
        } else {             
            //there is no organization when the user is being registered we will reset session including the organization there from auth/register
            //just after the organization is created
        }
    ...
    public function remember_user($identity) {
        ...
        if ($token->validator_hashed) {
            $this->db->update($this->tables['users'], ['remember_selector' => $token->selector,
                'remember_code' => $token->validator_hashed], [$this->identity_column => $identity]);
    ...
    public function login_remembered_user() {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, first_name, last_name, created_on, parent_id, permissions, payment_processor')
     ...
     */
    public function _generate_selector_validator_couple($selector_size = 40, $validator_size = 128) {
     ...
     */
    public function _retrieve_selector_validator_couple($user_code) {
        // Check code
        if ($user_code) {
    ...
    {
        return $this->db->where('id',$user_id)->update($this->tables['users'],['starter_step' => $step]);
    }
    public function getStarterStep($user_id)
    {
        return $this->db->from($this->tables['users'])->where('id',$user_id)->select('starter_step')->get()->row();
    }
    
    //$xorg_id can be an organization or a suborganization
    public function setUserCurrentOrganization($xorg_id, $type) {
        $session_data = $this->session->userdata();
        $user_id = $this->session->userdata('user_id');
        $this->load->model('organization_model');
        
        if($type == 'org') { //if is an organization            
            
            ///// ------ securing stuff, validating the organization sent belongs to user
            $orgIds    = getOrganizationsIds($user_id);
            $orgIdsArr = explode(',', $orgIds);
            if (!in_array($xorg_id, $orgIdsArr)) {
                throw new Exception('bad request');
            }
            ///// ------ 
            
            $org = $this->organization_model->get($xorg_id, 'church_name as name', false, $user_id);
            $session_data['currnt_org'] = ['orgnx_id' => (int) $xorg_id, 'sorgnx_id' => null, 'orgName' => $org->name, 'suborgName' => null, 'type' => $type];
        } elseif($type == 'sub') { //if is a suborganization
            ///// ------ securing stuff, validating the suborganization sent belongs to user
            $this->load->model('suborganization_model');            
            $suborg = $this->suborganization_model->get($xorg_id, $user_id, 'id, church_id, name');
            if(!$suborg) {
                throw new Exception('bad request');
            }
            ///// ------
            $org = $this->organization_model->get($suborg->church_id, 'church_name as name', false, $user_id);
            
            $session_data['currnt_org'] = ['orgnx_id' => (int) $suborg->church_id, 'sorgnx_id' => (int) $xorg_id, 'orgName' => $org->name, 'suborgName' => $suborg->name, 'type' => $type];
        } else {
